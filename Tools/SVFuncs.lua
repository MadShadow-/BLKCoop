...
function SW.ResetScriptingValueChanges()
	...
	SW.SV.GreatReset()
	for k,v in pairs(SW.SV.LogicBackUps) do
		SW.SV.SetLogicXMLProperty( k, v)
	end
end;
...
SW.SV.Data = {
	...
	["SettlerHealth"] = {1, 7791768, 13, true},
...
function SW.SV.GetLogicXMLProperty( _name)
	local data = SW.SV.LogicIndexByName[_name]
	if data == nil then Message("SetLogicXMLProperty: ".._name.." will not be set!") return -1 end
	if data[2] then -- value is int
		return SW.SV.GetLogicXMLPointer()[data[1]]:GetInt()
	else
		return SW.SV.GetLogicXMLPointer()[data[1]]:GetFloat()
	end
end
